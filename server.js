...
function buildSystemPrompt(){
  ...
  const mayuscula = it.mayuscula.join(', ');
...
Los siguientes términos deben ir siempre en cursiva (entre asteriscos): ${cursiva}.
Los siguientes términos llevan siempre mayúscula inicial: ${mayuscula}.
...
app.post('/api/italics/add', (req, res) => {
  try {
    ...
    if(tipo !== 'cursiva' && tipo !== 'mayuscula') return res.status(400).json({ error: 'Tipo inválido' });
