...
app.post('/api/ingredientes/add', (req, res) => {
  try {
    const { n, g, p, c, t, grupo, estado, fuente, usda_id } = req.body;
...
app.post('/api/ingredientes/edit', (req, res) => {
  try {
    const { nombre_original, n, g, p, c, t, grupo, estado, fuente, usda_id } = req.body;
    const ings = leerIngs();
...
app.get('/api/usda/search', async (req, res) => {
  ...
  try {
    const url = `https://api.nal.usda.gov/fdc/v1/foods/search?query=${encodeURIComponent(q)}&dataType=Foundation,SR%20Legacy&pageSize=10&api_key=${apiKey}`;
    const r = await fetch(url);
    const data = await r.json();
    if (!data.foods) return res.json({ results: [] });
...
app.post('/api/usda/update', async (req, res) => {
  ...
  try {
    const ings = leerIngs();
    const conId = ings.filter(i => i.usda_id);
    ...
    for (const ing of conId) {
      try {
        ...
        const c = Math.max(0, Math.round(((nutrients.c_total || 0) - (nutrients.fibra || 0)) * 100) / 100);
        const idx = ings.findIndex(i => i.usda_id === ing.usda_id);
        if (idx !== -1) {
